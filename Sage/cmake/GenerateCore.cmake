# Generate a core project that run on libs
option(IS_LIB_COMPILED_CORE_GEN "Compile Library and generate a core build target." ON)

if(IS_LIB_COMPILED_CORE_GEN)
    message(STATUS "Library Core Generation is enabled")

    IF(COMPILE_USING_OBJECT)
        add_library(SageEngine_Core STATIC
            ${OBJECT_LIST}
        )
        target_link_libraries(SageEngine_Core
            ${ALL_LIBS}
        )
        message("LIB_LIST: ${LIB_LIST}")
        target_include_directories(SageEngine_Core INTERFACE
            ${INCLUDES_LIST}
        )

        message("INCLUDES_LIST: ${INCLUDES_LIST}")

        set_property(TARGET SageEngine_Core PROPERTY CXX_STANDARD 20)

        IF(CMAKE_CXX_COMPILER_ID MATCHES "GNU|Clang")
            target_compile_options(SageEngine_Core PRIVATE -Wall ${DisableWarnings})
        elseif(MSVC)
            target_compile_options(SageEngine_Core PRIVATE /W4 /WX-)
        endif()

        add_dependencies(SageEngine_Core
            ${LIB_LIST}
        )
    ELSE()
        add_library(SageEngine_Core INTERFACE)
        target_link_libraries(SageEngine_Core INTERFACE
            ${LIB_LIST}
        )

        message("${LIB_LIST}")
        target_include_directories(SageEngine_Core INTERFACE
            ${INCLUDES_LIST}
        )
        message("${INCLUDES_LIST}")
    ENDIF()
endif()

# Generate a core project that run on libs
file(GLOB_RECURSE SageEngine_source_files
    ${CMAKE_SOURCE_DIR}/SageEngine/src/*.[ch]pp
    ${CMAKE_SOURCE_DIR}/SageEngine/include/*.[h]pp
    ${CMAKE_SOURCE_DIR}/SageEngine/include/*.h
)

file(GLOB SAGEENGINE_INC_SUBDIR
    ${CMAKE_SOURCE_DIR}/SageEngine/include/*/
)

file(GLOB SAGEENGINE_SRC_SUBDIR
    ${CMAKE_SOURCE_DIR}/SageEngine/src/*/
)
set(ENGINE_SUB_INC_FILES)
set(ENGINE_INCLUDES_LIST)

foreach(SAGEENGINE_INC_SUBDIRS ${SAGEENGINE_INC_SUBDIR})
    list(APPEND ENGINE_INCLUDES_LIST ${SAGEENGINE_INC_SUBDIRS})
    get_filename_component(SAGEENGINE_INC_SUBDIR_NAME ${SAGEENGINE_INC_SUBDIRS} NAME)
    file(GLOB SAGEENGINE_INC_SUBDIR_FILES
        ${SAGEENGINE_INC_SUBDIRS}/*.[h]pp
    )
    source_group("Header Files/${SAGEENGINE_INC_SUBDIR_NAME}" FILES ${SAGEENGINE_INC_SUBDIR_FILES})

    list(APPEND ENGINE_SUB_INC_FILES ${SAGEENGINE_INC_SUBDIR_FILES})
endforeach()

set(ENGINE_SUB_SRC_FILES)

foreach(SAGEENGINE_SRC_SUBDIRS ${SAGEENGINE_SRC_SUBDIR})
    get_filename_component(SAGEENGINE_SRC_SUBDIR_NAME ${SAGEENGINE_SRC_SUBDIRS} NAME)
    file(GLOB SAGEENGINE_SRC_SUBDIR_FILES
        ${SAGEENGINE_SRC_SUBDIRS}/*.[ch]pp
        ${SAGEENGINE_SRC_SUBDIRS}/*.h
    )
    source_group("Source Files/${SAGEENGINE_SRC_SUBDIR_NAME}" FILES ${SAGEENGINE_SRC_SUBDIR_FILES})
    list(APPEND ENGINE_SUB_SRC_FILES ${SAGEENGINE_SRC_SUBDIR_FILES})
endforeach()

add_executable(SageEngine
    ${SageEngine_source_files}
    ${ENGINE_SUB_SRC_FILES}
)

# remove main
list(REMOVE_ITEM SageEngine_source_files ${CMAKE_SOURCE_DIR}/SageEngine/src/main.cpp)
add_library(
    SageCore
    ${SageEngine_source_files}
    ${ENGINE_SUB_SRC_FILES}
)

# Link the executable with the necessary libraries
target_link_libraries(SageEngine
    ${ALL_LIBS}
    SageEngine_Core
)

target_link_libraries(SageCore
    ${ALL_LIBS}
    SageEngine_Core
)

# Add Executable for Editor
set_property(TARGET SageCore PROPERTY CXX_STANDARD 20)

set_property(TARGET SageEngine PROPERTY CXX_STANDARD 20)

IF(CMAKE_CXX_COMPILER_ID MATCHES "GNU|Clang")
    target_compile_options(SageEngine PRIVATE -Wall ${DisableWarnings})
elseif(MSVC)
    target_compile_options(SageEngine PRIVATE /W4 /WX-)
endif()

if(WIN32 AND FMOD_IMPORT_SUCCESS)
    add_custom_command(TARGET SageEngine POST_BUILD
        COMMAND ${CMAKE_COMMAND} -E copy_if_different
        "$<$<CONFIG:Debug>:${FMOD_LIB_DIR}/fmodL.dll>$<$<NOT:$<CONFIG:Debug>>:${FMOD_LIB_DIR}/fmod.dll>"
        $<TARGET_FILE_DIR:SageEngine>)
endif()

if(WIN32 AND MONO_IMPORT_SUCCESS)
    add_custom_command(TARGET SageEngine POST_BUILD
        COMMAND ${CMAKE_COMMAND} -E copy_if_different
        "$<$<CONFIG:Debug>:${MONO_DLL_PATH}>$<$<NOT:$<CONFIG:Debug>>:${MONO_DLL_PATH}>"
        $<TARGET_FILE_DIR:SageEngine>
    )
    add_custom_command(TARGET SageEngine POST_BUILD
        COMMAND ${CMAKE_COMMAND} -E copy_if_different
        "${MONO_DLL_POSIX}"
        $<TARGET_FILE_DIR:SageEngine>
    )
endif()

target_include_directories(SageEngine PRIVATE
    ${CMAKE_SOURCE_DIR}/SageEngine/include
    ${CMAKE_SOURCE_DIR}/SageEngine/include/internal
    ${ENGINE_SUB_INC_FILES}
    ${INCLUDES_LIST}
)

target_include_directories(SageCore PRIVATE
    ${CMAKE_SOURCE_DIR}/SageEngine/include
    ${CMAKE_SOURCE_DIR}/SageEngine/include/internal
    ${ENGINE_SUB_INC_FILES}
    ${INCLUDES_LIST}
)

# Gather source files for editor
file(GLOB_RECURSE SageEditor_source_files
    ${CMAKE_SOURCE_DIR}/SageEditor/src/*.[ch]pp
    ${CMAKE_SOURCE_DIR}/SageEditor/include/*.[h]pp
    ${CMAKE_SOURCE_DIR}/SageEditor/include/*.h
)

file(GLOB SAGEEDITOR_INC_SUBDIR
    ${CMAKE_SOURCE_DIR}/SageEditor/include/*/
)

file(GLOB SAGEEDITOR_SRC_SUBDIR
    ${CMAKE_SOURCE_DIR}/SageEditor/src/*/
)

set(EDITOR_SUB_INC_FILES)

foreach(SAGE_EDITOR_INC_SUBDIRS ${SAGEEDITOR_INC_SUBDIR})
    get_filename_component(SAGEEDITOR_INC_SUBDIR_NAME ${SAGE_EDITOR_INC_SUBDIRS} NAME)
    file(GLOB SAGEEDITOR_INC_SUBDIR_FILES
        ${SAGE_EDITOR_INC_SUBDIRS}/*.[h]pp
    )
    source_group("Header Files/${SAGEEDITOR_INC_SUBDIR_NAME}" FILES ${SAGEEDITOR_INC_SUBDIR_FILES})

    list(APPEND EDITOR_SUB_INC_FILES ${SAGEEDITOR_INC_SUBDIR_FILES})
endforeach()

set(EDITOR_SUB_SRC_FILES)

foreach(SAGEEDITOR_SRC_SUBDIRS ${SAGEEDITOR_SRC_SUBDIR})
    get_filename_component(SAGEEDITOR_SRC_SUBDIR_NAME ${SAGEEDITOR_SRC_SUBDIRS} NAME)
    file(GLOB SAGEEDITOR_SRC_SUBDIR_FILES
        ${SAGEEDITOR_SRC_SUBDIRS}/*.[ch]pp
        ${SAGEEDITOR_SRC_SUBDIRS}/*.h
    )
    source_group("Source Files/${SAGEEDITOR_SRC_SUBDIR_NAME}" FILES ${SAGEEDITOR_SRC_SUBDIR_FILES})
    list(APPEND EDITOR_SUB_SRC_FILES ${SAGEEDITOR_SRC_SUBDIR_FILES})
endforeach()

add_executable(SageEditor
    ${SageEditor_source_files}
    ${EDITOR_SUB_SRC_FILES}
)

target_link_libraries(SageEditor
    ${ALL_LIBS}
    SageCore
)

set_property(TARGET SageEditor PROPERTY CXX_STANDARD 20)

if(WIN32 AND FMOD_IMPORT_SUCCESS)
    add_custom_command(TARGET SageEditor POST_BUILD
        COMMAND ${CMAKE_COMMAND} -E copy_if_different
        "$<$<CONFIG:Debug>:${FMOD_LIB_DIR}/fmodL.dll>$<$<NOT:$<CONFIG:Debug>>:${FMOD_LIB_DIR}/fmod.dll>"
        $<TARGET_FILE_DIR:SageEditor>)
endif()

if(WIN32 AND MONO_IMPORT_SUCCESS)
    add_custom_command(TARGET SageEditor POST_BUILD
        COMMAND ${CMAKE_COMMAND} -E copy_if_different
        "$<$<CONFIG:Debug>:${MONO_DLL_PATH}>$<$<NOT:$<CONFIG:Debug>>:${MONO_DLL_PATH}>"
        $<TARGET_FILE_DIR:SageEditor>
    )
    add_custom_command(TARGET SageEditor POST_BUILD
        COMMAND ${CMAKE_COMMAND} -E copy_if_different
        "${MONO_DLL_POSIX}"
        $<TARGET_FILE_DIR:SageEditor>
    )
endif()

target_include_directories(SageEditor PRIVATE
    ${CMAKE_SOURCE_DIR}/SageEditor/include
    ${CMAKE_SOURCE_DIR}/SageEditor/include/internal
    ${EDITOR_SUB_INC_FILES}
    ${ENGINE_INCLUDES_LIST}
    "${CMAKE_SOURCE_DIR}/SageEngine/include"
    "${CMAKE_SOURCE_DIR}/SageEngine/include/internal"
    ${INCLUDES_LIST}
)
