...
namespace SageUIEditor
{
	...
	void Show_Console_Window()
	{
		...
		{
			SageMonoManager::STDOUTCS& output_stream = SageMonoManager::output_stream;
...
#pragma endregion
			...
			if (output.empty()) {
				...
				ImGui::Text("CS Output will go here");
			}
			for (auto& [line, num] : output)
			{

				ImGui::Dummy(ImVec2{ 0,10.f });
				ImGui::Text("%s [%d]", line.c_str(), num);
